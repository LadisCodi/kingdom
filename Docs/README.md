...
| 9 | [Relics and the collection](features/09-relics.md) | five relics as **permanent passives with no ceiling**, levelled by **card albums in a 14-day shared season** — packs, duplicates, the vault, trading, wildcards, the season hero | **redesigned 2026-09-09**; the build carries the old model |
...
| [`proposals/album-cycles.md`](proposals/album-cycles.md) | a **proposal**, not a spec: closing all five albums, claiming, and running the five again inside one season — which needs the nine cards SPENT on completion, or the loop never terminates |
| [`proposals/relic-effects.md`](proposals/relic-effects.md) | a **proposal**, not a spec: what each of the five relics does, level by level — a passive that never stops being worth having, and an active that is a placed ZONE on a cooldown, with exactly one of its four numbers growing with the relic's level |
