...
| **OQ-51** | **An iron vein and a mountain are now the same thing with different art.** Stone comes out of mountains, worked by the Quarry; an iron vein is a *rich* Stone node at 3 a tap, worked by the Mine. After the currency simplification made a vein pay Stone, the Mine became "a second Quarry pointed at a second stone feature" — and unifying stone with wood made that plainer rather than fixing it. **Three honest exits:** fold the vein into the mountain and retire the Mine (loses a district, a feature, a source, an upgrade and leaves Mining / Deep Mining without a building); keep the Mine but point it at mountains too, as a second parallel line — the precedent is the Barracks and the Spear Hall both training Lancers; or accept two rich stone nodes as flavour. **Deferred deliberately**, and the reason it is worth revisiting is that it is the one place where a *reduction* is available. | the stone line's shape | [`01-map-and-fog.md`](features/01-map-and-fog.md) §3.1 | — |
...
| **OQ-43** | **Late-game costs against scaled taps.** Taps scale with production and a watcher gathers ~50% faster — and six ad placements could double that. **A watcher progressing twice as fast as a non-watcher is a different game rather than a meaningful offer.** Whether the build-cost curve absorbs it is the first thing to look at. | [`14-monetization.md`](features/14-monetization.md) §6 |
| **OQ-44** | **Cell exhaustion becomes the ceiling on the Mana pool.** A 100-tap pool needs 10 fully exhausted Forest cells and the map has 17. Comfortable now; **it is what bites first if the pool grows again.** | [`08-magic.md`](features/08-magic.md) |
