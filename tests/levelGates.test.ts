...
describe('per-level housing capacity', () => {
  // One house holds TWO (Docs/onboarding.md step 13), and the Townhall itself
  // sleeps ONE — so the tutorial can hire a villager before it has built
  // anything, and the first house is room to grow rather than permission to
  // start.
  it('a level-2 house holds twice what a level-1 one does, on top of the Townhall bed', () => {
    const state = freshGame();
    const th = districtById(state, townhall(state).uniqueId)!;
    expect(districtCapacity(state, th)).toBe(1);
    expect(maxPopulation(state)).toBe(1); // a bed before a single building
  ...
  // "+1 to every district that houses anyone" — which now includes the
  // Townhall, since it has a bed. That follows from the rule as written
  // rather than being a special case for it.
  it('Communities adds +1 to every district that houses anyone', () => {
    const state = freshGame();
    addBuilt(state, 'Housing', HOUSE);
    addBuilt(state, 'Housing', { x: 0, y: -1 });
    expect(maxPopulation(state)).toBe(5); // 2 houses + the Townhall bed
    completeTech(state, 'Communities');
    expect(maxPopulation(state)).toBe(8); // +1 to each house AND the Townhall
    const house = state.city.districts.find((d) => d.definitionId === 'Housing')!;
    expect(districtCapacity(state, house)).toBe(3);
    expect(districtCapacity(state, districtById(state, townhall(state).uniqueId)!)).toBe(2);
  });
});
