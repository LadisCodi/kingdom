...
import { btn, costChips, iconEl } from './kit';
...
export function renderPlacementPanel(game: Game): HTMLElement {
  ...
  const short = game.shortfall(info.cost);
  const shortText = (Object.entries(short) as Array<[CurrencyId, number]>)
    .map(([c, n]) => `${n} ${c}`).join(' and ');

  const blockedBy = info.cell === null ? 'Nowhere legal to build it'
    : shortText ? `Short ${shortText}` : undefined;
  const build = btn({
    label: 'Build',
    kind: 'primary',
  ...
  return el('div', { class: 'plc-bar' },
    ...
    el('div', { class: 'plc-body' },
      ...
      verdict,
      el('div', { class: 'plc-cost' },
        costChips(info.cost, (c) => game.effectiveWalletValue(c)),
        el('span', { class: 'plc-time' },
